...
def can_dede_cevapla(user_prompt, chat_history, context_data, kaynak_var_mi):
    ...
    if kaynak_var_mi:
        ...
        3. Sonra elindeki kaynakları harmanlayarak detaylı, felsefi derinliği olan anlatımını yap.
        """
    else:
        gorev_tanimi = """
        GÖREVİN:
...
if prompt:
    ...
    with st.chat_message("assistant", avatar=CAN_DEDE_ICON):
        ...
        # --- YENİ MANTIK: KAYNAK VARSA, DETAY METNİ OLMASA BİLE LİNKLERİ GÖSTER ---
        if kaynak_var_mi:
            with detay_container.container():
                with st.expander("📜 Daha Fazla Detay ve Kaynaklar", expanded=False):
                    # Eğer yapay zeka detay metni oluşturduysa onu bas
                    if detay_text.strip():
                        st.markdown(detay_text)
                        st.markdown("\n---\n") # Ayırıcı çizgi
                    
                    # Kaynakları HER DURUMDA bas (Siteye trafik çekmek için)
